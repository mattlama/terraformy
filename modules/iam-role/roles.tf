...
data "aws_iam_policy_document" "role_document" {
  count   = var.create ? (length(var.role_type) > 0 ? (length(var.custom_policy) > 0 ? 0: 1) : 0) : 0
  version = "2012-10-17"
  statement {
    sid     = ""
    effect  = "Allow"
...
resource "aws_iam_role" "new_role" {
  count              = var.create ? (length(var.role_type) > 0 ? 1 : 0) : 0
...
resource "aws_iam_policy" "lambda_policy" {
  count       = var.create ? (length(var.role_type) > 0 ? (var.role_type[0] == "lambda" ? 1 : 0) : 0) : 0
